...
bool usePacked256(snpcoding method) {
  return method == Packed256 ? true : GLOBAL.genetics.any2bit;  
}

bool use256() { return true; }
...
bool usePacked256(snpcoding method) {
  if (GLOBAL.genetics.any2bit || method != Packed256) return false;
  ERR("'Packed256' is not available. Set 'RFoptions(any2bit=TRUE)'.");
}
